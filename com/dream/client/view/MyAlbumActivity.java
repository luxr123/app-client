...
import com.dream.client.adapter.AlbumGridViewAdapter;
import com.dream.client.util.ImageManager2;
...
public class MyAlbumActivity extends Activity {
	...
	private void initSelectImage() {
		...
		for (final String path : selectedDataList) {
			ImageView imageView = (ImageView) LayoutInflater.from(MyAlbumActivity.this).inflate(R.layout.choose_imageview,
					selectedImageLayout, false);
			selectedImageLayout.addView(imageView);
			hashMap.put(path, imageView);
			ImageManager2.from(MyAlbumActivity.this).displayImage(imageView, path, R.drawable.camera_default, 100, 100);
		...
		gridImageAdapter.setOnItemClickListener(new AlbumGridViewAdapter.OnItemClickListener() {
			...
			public void onItemClick(final ToggleButton toggleButton, int position, final String path, boolean isChecked) {
				...
				if (selectedDataList.size() >= 8) {
					...
					if (!removePath(path)) {
						Toast.makeText(MyAlbumActivity.this, "只能选择8张图片", 200).show();
					}
					return;
				}
